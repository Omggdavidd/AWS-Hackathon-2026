...
- `web/`: overview with three views on the same loops: List (home; rows grouped by time with the state on each row, hover actions, keyboard triage), Calendar (six-week grid on due dates) and Board (draggable state groups around a hub, layout saved in the browser); headline, live clock and the docked agent panel above them. White/navy themes and Inter are shared across the app. Loop detail shows facts, next action, consequence, evidence quotes and source links, actions with the effects the agent produced, and folded history; approve/decline with pending state, "I already did this", activity feed, and a message page behind every source id let each claim be checked against the original mail or event. Runs on the local ledger seeded from the demo.
