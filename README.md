...
`agentcore status --json`, run from `agent/`, prints the runtime ARN for `web/.env.local`. With both values set, **Scan inbox** runs the real agent over the demo inbox and the dashboard fills in as loops land (about 95 to 105 seconds), **Check for new mail** replays the next morning's batch, **Handle what you can** executes every action the risk gate allows, and **Approve** executes a single high-risk action.
