...
- [ ] Decide whether the take opens on the welcome screen. A fresh browser profile shows it on every route (four steps: your name, the agent's name, what the inbox is for, the inbox; then the five-stop tour); a profile that has visited before goes straight to Today. Either is fine; do not let the tour run mid-scene.
