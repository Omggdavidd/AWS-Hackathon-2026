...
|---|---|---|
| `web/` (Next.js) | Interactive whiteboard with the four states (browser-only layout coordinates), loop detail with evidence and timeline, message pages behind every source id, approval queue, activity feed, command bar, "catch me up"; Google OAuth callback and token storage; invoking the agent runtime server-side | Call Bedrock or Google APIs from the browser; hold business rules about state transitions |
