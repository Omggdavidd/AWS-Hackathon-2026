...
- High-risk execution is refused in code unless the `ProposedAction` is `APPROVED`. Prompts cannot override this, and the schema refuses a high-risk action that claims it needs no approval, so such a record never reaches the ledger.
