...
- **Sending mail, paying, submitting a form and booking a paid service never execute unattended**, whatever risk tier they carry, along with the catch-all `other`. The tier is assigned by the Risk Judge, so it is model output; resting the guarantee on the tier alone meant a `pay` the model rated `low` was auto-executable (#163), and a `book_appointment` the model rated `medium` went the same way (#173). The type is now checked before the tier in `isAutoExecutable`, and a test pins each of those five types at all three tiers. Approval still executes them — this is a gate, not a ban.
