...
**Mitigated:** the three routes now reject any request that did not come from the app's own origin (`web/lib/same-origin.ts`). Browsers always send `Origin` on a POST, so the app works unchanged; `curl` and most bots send nothing and get a 403. This also closes the CSRF hole — before it, any page a teammate visited could have fired a scan from their browser. Next's Server Actions already do an equivalent check of their own, which is why the buttons on the loop page were never exposed the same way.
...
2. **A rate limit on the three routes** — per-IP and a global daily ceiling. Needs shared state, so on Vercel it means a counter in DynamoDB or a managed limiter. Deliberately not attempted the day before submission: a limiter that misfires locks a judge out of the demo, which is worse than the risk it removes.
...
The `openloop-web` user for Vercel is scoped to DynamoDB item and query actions on the table plus `bedrock-agentcore:InvokeAgentRuntime` on the runtime ARN (#19). **Unverified here** — auditing it needs console access this review did not have. Someone with credentials should confirm it matches that description, and in particular that it carries no `dynamodb:DeleteTable` and no wildcard resource.
