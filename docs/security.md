...
Two things bound the damage, neither of them completely, in priority order:
...
2. **A rate limit on those routes.** There is one now, and it is worth being precise about what it does and does not do. `withinDailyCeiling` (`web/lib/ceiling.ts`) counts every invocation of a paid path — `/api/scan`, `/api/handle`, `/api/catch-up`, `/api/ask` and `approveAction` — into one DynamoDB row per UTC day and refuses past 500. `OPENLOOP_DAILY_INVOCATIONS` overrides the number.

   - **It fails open, deliberately.** No table configured, an unreachable or throttled table, a response without the counter in it: every one of those lets the request through (`ceiling.ts:50-52`). Locking a judge out of the live demo mid-evaluation would cost more than the abuse this prevents. The price of that choice is that a broken counter and an unused one look the same from outside.
   - **It is per deployment, not per IP.** There is no principal to count against — see the first section — so there is nothing to count but the deployment as a whole. One attacker spends the day's allowance in a minute and everybody else gets a refusal until midnight UTC. It turns unbounded spend into a denial of service against ourselves, which is the better of the two and still not a good outcome.
   - **It bounds runaway spend; it does not enforce the budget.** 500 invocations at the price of a scan is about $250, ten times the $25 alert above it. That is an upper bound — catch-up and ask cost a fraction of a scan — but the ceiling is a stop on the runaway case, not a limit that keeps the bill inside what was budgeted. A number low enough to do that would refuse honest use.
   - **Two things about it are unverified.** The counter row carries a `ttl` attribute, but TTL is not enabled on `openloop-ledger`: `packages/ledger-dynamo/scripts/create-table.ts` has no `TimeToLiveSpecification`, so the rows accumulate at one a day and are never collected. Harmless in volume, wrong as written. The one that matters: **nothing has read a `RATE#` row back from the deployment.** The counter needs `dynamodb:UpdateItem` on the table, which the IAM section below records `openloop-web` as holding; if that were narrowed, or the write failed for any other reason, the ceiling would fail open on every request from then on and present no symptom at all. A limiter that has been silently inert since the day it deployed is indistinguishable from a working one unless somebody checks that `PK = RATE#<today>` exists in the table after the app has been used. Nobody has.
...
| Abuse-prone and paid endpoints are rate-limited per route | **Partly met.** One ceiling of 500 invocations a UTC day across all five paid paths, per deployment rather than per route or per caller, and it fails open on any counter error. It stops a runaway script; it does not keep the bill inside the budget, and it has never been observed counting on the deployment. |
...
| Fail-closed is chosen deliberately and written down; fail-open is written down too | **This page is that.** The fail-open choices — no auth, a daily ceiling that fails open, no CSP, a trusted proxy header — are each named above with the reason. |
...
The one thing that would *not* be quick: there is no kill switch for the paid routes short of unsetting `OPENLOOP_RUNTIME_ARN` in Vercel and redeploying, which makes every agent button return 503. That is the break-glass move if the bill starts climbing. Setting `OPENLOOP_DAILY_INVOCATIONS` to `1` is the gentler version and costs the same redeploy; `0` disables the ceiling rather than closing it (`ceiling.ts:46`), so it is the wrong number to reach for in a hurry.
