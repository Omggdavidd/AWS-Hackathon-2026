...
`test/log.test.ts` pins that. Nor does the Google access token reach the log: it travels in the
`Authorization` header only, and a refused request is named by its status and by a URL stripped of
its query.
