...
describe('needsDecision', () => {
  ...
  it('asks the policy gate, so a type no tier makes safe is a decision', () => {
    // A low-risk follow_up passes both of the old tests and is still refused by `mayExecute`:
    // before this it fell through Handle into "needs you" and out of the Decisions list.
    expect(needsDecision(action('fu', { type: 'follow_up', riskTier: 'low' }))).toBe(true)
