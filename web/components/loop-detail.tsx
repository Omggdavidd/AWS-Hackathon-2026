 ...
 * every action works the same in both places.
 */
export async function LoopDetail({ id, mode }: { id: string; mode: 'page' | 'pane' }) {
  const store = await getStore()
  const loop = await store.getLoop(USER_ID, id)
  if (!loop) {
    if (mode === 'page') notFound()
    return <p className="pane-missing">That loop is no longer in the ledger.</p>
  }
